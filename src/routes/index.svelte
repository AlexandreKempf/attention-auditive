  ...
  <div class="grid grid-rows-18 gap-2 grid-flow-col">
    ...
    <div>
      <div id={word.id} on:click={updateIndexMouse} class="justify-self-stretch pt-1 pb-1 pl-3 {word.id == index ? 'border-gray-500' : 'border-gray-200'} border self-stretch relative rounded-md shadow-sm">
        <p id={word.id} on:click={updateIndexMouse} class="block w-full pr-10 sm:text-sm rounded-md {word.text == 'rouge' ? 'text-red-600 font-semibold' : 'text-gray-600'}">
          {word.text}
        </p>
...
<div>
  <label for="comment" class="text-sm font-medium text-gray-500"> Commentaires </label>
  <textarea id="comment" class="w-full h-full text-sm mr-5 mb-3 shadow-sm focus:ring-indigo-500 focus:border-indigo-500 border-gray-300 rounded-md" on:focus={onFocus} on:blur={onBlur} bind:value={comments}></textarea>
</div>
